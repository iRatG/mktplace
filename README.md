...
| 14. Внутренние тикеты + опросники для бизнеса | ✅ | apps/business_queries: Ticket для группы "IT Team" (замена переписки в Telegram); BusinessQuery — опросники A/B по ссылке-токену + общему паролю для бизнес-заказчиков без аккаунта, с историей ответов и rate limit на пароль |

Подробные спецификации по каждому модулю — в `openspec/specs/<app>/spec.md`.
